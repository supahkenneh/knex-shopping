...
router.delete('/:user_id/:product_id', (req, res) => {
  ...
  db.raw('SELECT * FROM cart WHERE user_id = ? AND products_id = ?', [userId, productId])
    .then(result => {
      if (!result || !result.rowCount) {
    ...
    .then(result => {
      return db.raw('DELETE FROM cart WHERE user_id = ? AND products_id = ? RETURNING *', [userId, productId]);
    })
    .then(result => {
